export type TokenKind = 'number'|'string'|'identifier'|'path'|'keyword'|'operator'|'punct'|'eof';
export interface Token { kind: TokenKind; value: string; line: number; column: number; }

// ARR deliberately has no Kotlin/Swift compatibility vocabulary. React is an influence on
// declarative composition only; the language semantics below are ARR-native.
const keywords=new Set([
  'import','from','fn','if','else','while','for','return','boot','const',
  'int','int8','int16','int32','int64','uint8','uint16','uint32','uint64',
  'float','double','string','bool','any','component','state','render',
  'signal','watch','emit','derive','view','true','false','null','kernelPanic','reboot'
]);

export class Lexer {
 private i=0; private line=1; private column=1;
 constructor(private readonly source:string){}
 lex():Token[]{const out:Token[]=[];while(!this.atEnd()){
   const c=this.peek();
   ...
   if(['==','!=','<=','>=','&&','||','+=','-=','*=','/=','++','--','=>','?.','::',':='].includes(two)){this.advance();this.advance();out.push({kind:'operator',value:two,line,column});continue;}
 ...
 private atEnd(){return this.i>=this.source.length;} private peek(n=0){return this.source[this.i+n]??'\0';}
